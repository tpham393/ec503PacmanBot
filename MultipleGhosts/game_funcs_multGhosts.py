...
random_ghost = True; # True - Ghost movement random, False - Ghost chases you

def validGhostMove(x, y, ghost_x, ghost_y, gridsize):
    '''
    Given new ghost location and locations of the other ghosts, see if new location is valid.
    '''
    # Check if ghost within grid bounds
    if (x>=gridsize[0] or x<=-1): 
        return False;
    if (y>=gridsize[1] or y<=-1):
        return False;
...
def ghost_move(ghost_x, ghost_y, gridsize): 
    ...
    for i in range(len(ghost_x)): # Get random move for each ghost
        ...
        potentialMoves = []; 
        if validGhostMove(t_y[i], t_y[i]-1, modGhost_x, modGhost_y, gridsize):
            potentialMoves.append(1);
        if validGhostMove(t_x[i]+1, t_y[i], modGhost_x, modGhost_y, gridsize):
            potentialMoves.append(2);
        if  validGhostMove(t_x[i], t_y[i]+1, modGhost_x, modGhost_y, gridsize):
