...
if __name__ == '__main__':
    ...
    ghostType = ['Chase','Random'] # 'Chase' or 'Random'
    ###########################################################################

    # Init
    game = Game();
    env = PacmanEnv(num_ghosts=2, ghost_type=ghostType, grid_len=6, pellet_x=4, pellet_y=1, grid=game.grid);
    ...
    if 'Chase' in ghostType:
        ...
        f = open("policyIter_RandomRandom.txt", "w");
    for val in policy:
      f.write(str(val)+'\n');
    f.close();
